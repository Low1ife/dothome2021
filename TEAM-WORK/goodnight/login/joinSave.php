...
<?php
    ...
    function msg($alert, $ck){
        ...
        if($ck==1){
            $link = "location.href='http://goodnight.dothome.co.kr/goodnight/login/login.php'";
        }else{
            $link = "history.back()";
        }
        echo "<script>
    ...
    //비밀번호 검사
    if( $youPass !== $youPassC ){
        msg("비밀번호가 일치하지 않습니다. 다시 한번 확인해주세요!", 0);
        exit;
    }
    ...
    if( $check_name == false ){
        msg("이름이 정확하지 않습니다. 한글로만 적어주세요!", 0);
        exit;
    }
    ...
    if( $check_birth == false ){
        msg("생년월일이 정확하지 않습니다. 올바른 생년월일(YYYY-MM-DD)을 적어주세요!", 0);
        exit;
    }
    ...
    if( $check_number == false ){
        msg("번호가 정확하지 않습니다. 올바른 번호(000-0000-0000)를 적어주세요!", 0);
        exit;
    }
    ...
    if( $result ){
        ...
        if( $count == 0 ){
            ...
            msg("이미 회원가입을 했네요!! 로그인을 해주세요!!", 1);
            exit;
        }
    } else {
        msg("에러발생01 - 관리자에게 문의하세요!!", 0);
        exit;
    }
    ...
    if( $isEmailCheck = true && $isPhoneCheck = true ){
        ...
        if( $result ){
            msg("회원가입을 축하합니다. 로그인 해주세요!!!", 1);
        } else {
            msg("에러발생03 - 관리자에게 문의하세요!", 0);
            exit;
        }
    } else {
        msg("이메일 또는 핸드폰 번호가 틀립니다. 다시 한번 확인해주세요!!", 0);
        exit;
    }
?>  
                </div>
